...
public class KeyHandler implements KeyListener {
	...
	public void update() {
		...
		lookVec3(rotation, 0.05);

		p.obj.rotationVec[1] = p.obj.rotationVec[1] % (Math.PI * 2);

		Double yaw = p.obj.rotationVec[1];
		...
		if (MathUtil.dot(moveDir, moveDir, 1) > 0) {
			...
			p.obj.translationVec = MathUtil.vec3.add(p.obj.translationVec,
	...
	public void lookVec3(Double[] vec3, double lookspeed) {
		if (MathUtil.dot(vec3, vec3, 1) > 0) {
			p.obj.rotationVec = MathUtil.vec3.add(p.obj.rotationVec,
